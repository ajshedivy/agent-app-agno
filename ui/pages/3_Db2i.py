            ...
            with st.spinner("Processing your request..."):
                try:
                    ...
                    async with db2i_agent_session(
                        ...
                        logger.debug(
                            f"Adding {len(st.session_state.agent_memory)} previous messages to agent memory"
                        )

                        # We only need to add previous messages, not the current user message
                        for i, msg in enumerate(
                            st.session_state.agent_memory[:-1]
                            ...
                            if i == 0:  # Only add prefix to the first message
                                prefixed_content = (
                                    "[Previous conversation] " + prefixed_content
                                )
                    ...
                    logger.error(
                        f"Error during agent execution: {str(e)}", exc_info=True
                    )
